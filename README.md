...
* 📝 **詳細なログ出力**
  検索の進捗やエラー内容がコマンドプロンプトにリアルタイムで表示され、状況把握が容易になりました。

* 🛡️ **ウィンドウ保持機能**
  エラーでサーバーが停止しても、原因を確認できるまでウィンドウが閉じないように改善しました。

* 📂 **GUIフォルダ選択**
  Windows標準のフォルダ選択ダイアログから、直感的に検索対象のフォルダを選べるようになりました。

* 🚀 **安定性と精度の向上**
  設定ファイルのBOM対応や、AIへの提示情報の最適化（お気に入りファイルの優先度アップ）を行いました。
